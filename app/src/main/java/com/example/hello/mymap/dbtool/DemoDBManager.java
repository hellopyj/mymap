package com.example.hello.mymap.dbtool;
...
import com.example.hello.mymap.domain.InviteMessage;
import com.example.hello.mymap.domain.InviteMessage.InviteMesageStatus;
import com.example.hello.mymap.domain.RobotUser;
import com.example.hello.mymap.map.dbtools.MapMarkerDao;
...
public class DemoDBManager {
    ...
    * POint列表
    * */
    synchronized public List<MyMarkerOption> getMarkerOptionList() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        List<MyMarkerOption> markeroptions = null;
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + MapMarkerDao.TABLE_NAME, null);
            if(cursor.getCount()>0){
                markeroptions= new ArrayList<MyMarkerOption>();
            };
            while (cursor.moveToNext()) {

                //int id=cursor.getString(cursor.getColumnIndex(MapMarkerDao.COLUMN_NAME_ID));
               // MyMarkerOption myBaseMarker=new MyMarkerOption().type(cursor.getInt(cursor.getColumnIndex(MapMarkerDao.COLUMN_NAME_TYPE)))
                 //       .content(cursor.getString(cursor.getColumnIndex(MapMarkerDao.COLUMN_NAME_CONTENT)));
...
//                }
               ...
               // users.put(username, user);
                //markeroptions.add(myBaseMarker);
            }
            cursor.close();
        }
