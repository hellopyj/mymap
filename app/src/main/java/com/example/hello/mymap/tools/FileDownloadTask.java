...
public class FileDownloadTask extends AsyncTask<Void, Long, Boolean> {
    ...
    private String url;
    private File target;
    boolean overcovery;//是否覆盖
    boolean downing=true;
    //开始下载时间，用户计算加载速度
    private long previousTime;

    public FileDownloadTask(String url, String target, FileDownloadCallback callback,boolean overcovery) {
        this.url = url;
        this.okHttpClient = MyApplication.httpClient;
        this.callback = callback;
        this.target = new File(target);
        this.overcovery=overcovery;
